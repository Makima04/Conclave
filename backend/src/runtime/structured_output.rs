...
/// Validate that a parsed JSON value contains required string fields.
pub fn require_string_fields(value: &serde_json::Value, fields: &[&str]) -> Result<(), String> {
    let obj = value
        .as_object()
...
/// Validate that a parsed JSON value contains required array fields.
pub fn require_array_fields(value: &serde_json::Value, fields: &[&str]) -> Result<(), String> {
    let obj = value
        .as_object()
        .ok_or_else(|| "Expected JSON object".to_string())?;
    for field in fields {
