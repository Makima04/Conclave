...
> 定义平台四种内容包（Character Pack、World Pack、Agent Graph Pack、Plugin Pack）的结构、manifest schema、资源约定、版本规则、导入流程和校验机制。内容包是创作者入口，也是 SillyTavern 迁移的基础。
>
> **实现状态**：当前平台以扁平表存储（`character_cards` + `import_reports` + `world_books`）为主，ZIP 包导入/导出、目录结构约定、SemVer 版本管理尚未实现。Character Pack 和 World Pack 已有实际导入路径（单文件上传 + ImportWorkbench UI），Agent Graph Pack 和 Plugin Pack 为规划中。
...
| 功能 | 状态 | 说明 |
|---|---|---|
| Character Pack 导入 | 已实现 | 单文件 PNG/JSON 上传 + ImportWorkbench UI 确认。存储于 `character_cards` 表。 |
| World Pack 导入 | 已实现 | SillyTavern 世界书 JSON 上传。存储于 `world_books` 表。 |
| Agent Graph Pack | 未实现 | 规划中，目录结构和 graph.json 待定。 |
| Plugin Pack | 未实现 | 规划中，插件系统待设计。 |
| ZIP 包导入/导出 | 未实现 | 当前仅单文件上传，无 ZIP 打包/解压。 |
| 目录结构约定 | 未实现 | 存储为扁平表记录，非文件系统目录。 |
| SemVer 版本管理 | 未实现 | 导入时生成版本标识，但无 SemVer range 校验。 |
| 8 步校验链 | 部分实现 | PNG metadata 解析、regex 执行、HTML 拆分已实现；资源重写、JS 转译、依赖检查等待实现。 |
| Manifest Schema | 已实现 | 实际字段与本文档规范有差异（见下文）。 |
| ImportWorkbench UI | 已实现 | 前端 `ImportWorkbench.tsx` 提供导入预览、规则轨迹查看、诊断信息和确认/取消操作。 |
| LLM 辅助导入 | 已实现 | 后端支持 LLM 辅助 action 语义标注和 fallback schema 生成。 |

### 实际 Manifest Schema（与规范差异）

实际 `PackageManifest` 接口（前端 `types.ts` / 后端 `types.rs`）：

```ts
interface PackageManifest {
  pack_type: string;          // "character" 等
  id: string;
  name: string;
  version: string;
  source: string;             // 原始来源（如 "sillytavern"）
  source_hash: string;        // sha256 hash
  importer_version: string;   // 导入器版本
}
```

与本文档规范差异：

- 缺少 `author`、`description`、`platform_version`、`dependencies`、`assets` 字段（规划中）。
- 多出 `source`、`source_hash`、`importer_version` 字段（已实现）。

### 实际存储模型

当前使用扁平数据库表，非文件系统目录：

- `character_cards` 表：存储角色卡元数据、`conclave_package` JSON（含 ui/greetings/variables/state_schema/state_adapter/actions/compatibility）、原始卡 JSON。
- `import_reports` 表：存储导入报告（stages/rule_traces/diagnostics）。
- `world_books` 表：存储世界书条目（ST world_info 格式）。

不存在 `content_packs` 表或按 `pack_type/id/version/` 组织的文件系统路径。
...
| 字段 | 类型 | 必填 | 状态 | 说明 |
|---|---|---|---|---|
| `pack_type` | `character`、`world`、`graph`、`plugin` | 是 | 已实现 | 包类型，决定目录结构和校验规则。 |
| `id` | string | 是 | 已实现 | 全局唯一标识，建议格式 `<type>_<name>`，只允许 `[a-z0-9_]`。 |
| `name` | string | 是 | 已实现 | 人类可读名称。 |
| `version` | string | 是 | 已实现 | SemVer 格式，见下方版本规则。实际未做 range 校验。 |
| `source` | string | 否 | 已实现 | 原始来源（如 `sillytavern`、`rentry`、`custom_json`）。规范外新增。 |
| `source_hash` | string | 否 | 已实现 | 原始卡内容 sha256 hash，用于去重和审计。规范外新增。 |
| `importer_version` | string | 否 | 已实现 | 导入器版本标识。规范外新增。 |
| `author` | string | 否 | 待实现 | 创作者标识。 |
| `description` | string | 否 | 待实现 | 一句话描述。 |
| `platform_version` | string | 否 | 待实现 | 兼容的平台版本范围，SemVer range 格式。 |
| `dependencies` | string[] | 否 | 待实现 | 依赖的其他包 id。 |
| `assets` | string[] | 否 | 待实现 | 包内资源文件路径列表。 |

### 版本规则

> 待实现。当前导入时生成版本标识（如 `"0.1.0"`），但无 SemVer range 校验、版本兼容性检查或增量更新机制。
...
> 实现状态：Character Pack 已通过单文件导入（PNG/JSON）+ ImportWorkbench UI 实现。以下目录结构为规划中的完整规范，当前以 `character_cards` 表 + `conclave_package` JSON 存储。
...
> 待实现。当前角色人格信息存储在 `character_cards.description` / `character_cards.personality` 等扁平字段中，未拆分为 `personality.json` 结构。
...
> 待实现。当前关系数据通过导入器抽取到 `state_schema` / `state_adapter` 中管理，无独立 `relationships.json` 文件。
...
> 待实现。当前所有角色描述存储在单一字段中，未分离 `public.md` / `hidden.md`。导入时全部内容存入 `character_cards.description`。
...
> 实现状态：World Pack 已通过 SillyTavern 世界书 JSON 导入实现，存储于 `world_books` 表。以下目录结构和文件约定为规划中的完整规范。

世界包定义 RP 世界的地点、势力、规则、时间线和导演强度配置。

### 目录结构

> 待实现。当前世界书以 ST world_info 条目格式存储于 `world_books` 表，无独立 `lore/`、`timeline.json`、`rules.json`、`director_config.json` 文件。
...
> 当前实现：单文件上传（PNG/JSON），通过后端导入器（`orchestrator.rs`）处理，前端 ImportWorkbench UI 预览确认。以下 8 步校验链为规划中的完整规范，当前仅部分步骤已实现。
...
8. 写入
   └─ 写入 `character_cards` / `world_books` 表
   └─ 存储 `conclave_package` JSON
   └─ 返回导入报告（stages + diagnostics）
```
...
当前导入流程通过前端 `ImportWorkbench.tsx` 提供交互式确认：

1. 用户上传 PNG/JSON 文件。
2. 后端导入器（`orchestrator.rs`）执行解析 -> regex -> HTML 拆分 -> 资源扫描 -> JS 分析 -> action 抽取 -> 变量抽取 -> 状态转换层生成。
3. 返回 `ImportDraftResponse`，包含 `package_draft`（ConclaveCardPackage）和 `import_report`（ImportReport）。
4. 前端 Workbench 展示：
   - 卡包预览（greetings、UI type、variables、actions）。
   - 导入报告（stages 状态、rule_traces 轨迹、diagnostics 诊断）。
   - API 兼容性信息。
5. 用户确认后调用 `confirmImport` 写入数据库，或取消丢弃。
...
```text
1. 读取 character_cards / world_books 表
2. 按包类型校验内部一致性
3. 打包为 ZIP（保持目录结构）
...
> 实现状态：当前导入器直接将 ST 角色卡字段映射到 `ExternalCard` 中间结构，再转换为 `ConclaveCardPackage`。以下文件级映射为规划中的 Character Pack 规范，当前实现未生成独立的 `public.md`、`personality.json`、`relationships.json` 等文件。

SillyTavern 的角色卡、世界书和预设可以通过结构化映射转换为平台内容包。

### 角色卡 → Character Pack

当前实际映射路径：ST 卡 JSON/PNG -> `ExternalCard`（18 字段中间结构）-> `ConclaveCardPackage`。
...
| 风险 | 影响 | 缓解措施 | 状态 |
|---|---|---|---|
| 公开/隐藏设定划分不当 | NPC 泄露不该知道的秘密，或过于封闭导致无法自然交流。 | 导入后提供可见性预览工具，让用户检查 NPC 在特定场景能看到哪些信息。 | 待实现（当前无分离） |
| 资源文件体积失控 | 导入包过大，影响本地存储和加载速度。 | 资源大小限制 + 导入时警告 + 运行时懒加载。 | 待实现（仅 URL 检测） |
| Agent Graph Pack prompt 注入 | 恶意 prompt 可能绕过权限、泄露隐藏信息或操纵其他 Agent。 | prompt 模板只在节点自己的 ContextBundle 内使用，不能访问其他节点的上下文；所有输出经过 Runtime schema 校验。 | 待实现 |
| 版本兼容性断裂 | 新版平台不兼容旧包格式，或旧平台无法解析新包。 | manifest 声明 `platform_version`，导入时严格检查；不兼容时阻止导入并提示原因。 | 待实现 |
| SillyTavern 迁移信息丢失 | 自动迁移无法完美分离公开/隐藏信息，触发条件逻辑简化。 | 导入报告明确列出诊断信息，引导用户手动审查 ImportWorkbench。 | 已实现（ImportWorkbench） |
| 依赖包缺失 | Agent Graph Pack 引用的 Character Pack 未导入。 | 导入时检查依赖，缺失时阻止导入并列出缺失依赖。 | 待实现 |

---

## 验收测试

> 以下测试场景标注了实现状态。标注"待实现"的测试依赖尚未完成的功能。

| 测试场景 | 通过标准 | 状态 |
|---|---|---|
| 导入合法 Character Pack | PNG/JSON 解析、regex 执行、HTML 拆分通过，写入 `character_cards` 表，可被会话引用。 | 已实现 |
| 导入缺少 manifest 的 ZIP | 返回 `error`，阻止导入，明确提示缺少 `manifest.json`。 | 待实现（无 ZIP 导入） |
| 导入含路径逃逸的 ZIP | 返回 `error`，阻止导入，提示不安全路径。 | 待实现（无 ZIP 导入） |
| Agent Graph Pack 权限声明 | 导入时展示所需权限列表，用户未授权时阻止导入。 | 待实现 |
| 角色包公开/隐藏分离 | NPC ContextBundle 包含 `public.md` 内容，不包含 `hidden.md` 中 NPC 未知的秘密。 | 待实现 |
| 版本兼容性检查 | 声明 `platform_version: ">=2.0.0"` 的包在平台 1.x 版本导入时被拒绝。 | 待实现 |
| 导出再导入一致性 | 导出的 ZIP 重新导入后，所有文件和 manifest 内容与原始包一致。 | 待实现（无导出） |
| SillyTavern 角色卡迁移 | 导入 SillyTavern 格式角色卡后，生成 `ConclaveCardPackage`，导入报告列出诊断信息。 | 已实现 |
| ImportWorkbench 预览 | 导入后在 Workbench 中可查看 package draft、规则轨迹和诊断信息。 | 已实现 |
| 依赖检查 | Agent Graph Pack 声明依赖但目标 Character Pack 未导入时，返回缺失依赖错误。 | 待实现 |
| 资源完整性校验 | manifest 声明的资源文件缺失时，返回 `warning` 或 `error`（取决于缺失文件是否必需）。 | 待实现（仅资源 URL 检测） |
