...
`Dynamic Master` · `4-Layer Pipeline` · `Turn State` · `Agent Lifecycle` · `Context Isolation` · `Plan Validator` · `Knowledge Boundary`
...
Master Agent 不再是固定图中的一个节点，而是每轮都运行的调度中心。它决定本轮调用哪些 Actor Agent、是否激活 User Agent 自动代理、是否创建新 NPC、是否冷却闲置 NPC。User Agent 是固定 Actor，不能被新增、删除或冷却。
...
│  按 Master 计划调用 Actor Agent（User / NPC）                    │
...
**解析 Agent (Parser)** -- 可选 Agent，通过 `parser_enabled` 配置控制。禁用时用户输入直接进入 Master。
...
| `final_writer_id` | `string?` | 可选。指定多 Writer 场景下产生最终叙事的 Writer ID |

### Plan Validator

Master 计划生成后，经过 `plan_validator::validate_plan` 校验：

1. 移除不在活跃列表中的 Agent 调用。
2. 截断超过 2000 字符的 task。
3. 校验 `inject_from` 依赖是否指向已有的 Agent 或更早的调用。
4. 调用数 cap 至 `max_active_agents`。
5. 阻止删除永久 Agent（Master/Parser/Writer）。
6. lifecycle 操作数限制为 5。

无效调用被静默移除（不拒绝整个计划），并记录 warning 日志。

### Runtime 自动注入 State Agent

若 session 存在 `_state_agent_writable` 变量且 Master 计划中未包含 State Agent，Runtime 会自动将 State Agent 注入 DAG，依赖所有 NPC/User Agent 的输出。State Agent 通过 `update_variables` tool call 直接持久化变量变更。

### Runtime 自动注入 User Agent（`user_auto`）

当 Master 计划设置 `user_auto=true`（战斗、需要自动补完玩家物理动作的回合，或玩家明确放权）且计划未显式调用 User Agent 时，Runtime 会自动将 User Agent 注入 DAG：

- `inject_from` = 本轮被调用的 NPC 列表（让玩家补完时能看到 NPC 的行为）
- 把 User Agent id 追加进 Writer 的 `inject_from`，使 Writer 合成玩家动作

> **DAG 无环约束**：本文档 Scene 2 描述的战斗「User Agent 与 NPC 互相注入」在运行时被 `compile_dag` 简化为 **NPC → User Agent → Writer** 的串行依赖（DAG 不允许互相依赖的环）。普通对话 `user_auto=false` 时 User Agent 保持休眠——用户本人即玩家角色，无需模拟。
...
5. **DAG 并发执行**：调用列表编译为 DAG，同一层级的 Agent 并发执行，**并发上限 `MAX_CONCURRENT_AGENTS=4`**（Semaphore 限流）
6. **Per-Agent 模型覆盖**：优先级为 `agent.config.model` → session `sub_agent_model` → 会话主模型 → 全局默认模型

> **会话主模型（`session.config.model`）**：`SessionConfig.model` 字段是 per-session 的对话主模型（单 agent 的 Chat LLM，或多 agent 的基础模型）。`execute_turn` / `execute_turn_stream` / `regenerate_turn` 优先用它，为空时回退到 `provider_configs.is_default=1` 的全局默认模型（`load_provider_model`）。格式同其它模型引用：`provider:<id>:<model>`、裸模型名、或空串表默认。`variable_tool_model`（变量解析 LLM）独立解析，空时同样回退全局默认。
...
## Layer 4: 后处理层 (Post-Turn)

Writer 输出最终叙事文本后，执行多项后处理操作。后处理在主事务 commit 后执行，失败不影响主事务。

### State Agent（Variable Tool Agent）

State Agent 在 DAG 执行阶段（Layer 3）运行，**不在 Layer 4**。它通过 `update_variables` tool call 直接持久化变量变更到 `state_snapshots` 表，**无 pending_proposals 审批，无风险分级门控**。

- Runtime 在 DAG 编排时自动注入 State Agent（当 `_state_agent_writable` 非空且 Master 未包含时）
- State Agent 依赖所有 NPC/User Agent 的输出
- tool call 提取后立即 commit 到 `state_snapshots`

### Compression（异步 post-commit）

独立的压缩流程分析本轮交互，生成结构化状态更新：

- 接收：用户输入 + Writer 最终叙事 + 当前 ContextBundle
- 输出：CompressionResult，包含：
  - `scene_summary`：覆盖式场景状态摘要
  - `events` + `structured_events`：本轮有意义事件
  - `foreshadowing`：伏笔状态更新
  - `state_changes`：直接写入 `state_snapshots`
- 持久化到：`turn_summaries`、`memory_events`、`foreshadowing`、`state_snapshots`
- 也可通过 `turn_jobs` 后台队列异步执行（如 recompression 场景）

### Knowledge Extraction（异步 post-commit）

从用户输入 + 最终叙事中抽取"谁知道什么"事件（`KnowledgeEvent`），写入 `agent_knowledge_events` 表。详见 Agent 边界与权限。

### 为什么分离压缩和知识抽取

- **关注点分离**：Writer 专注叙事，压缩 Agent 专注信息压缩，知识系统专注边界追踪
- **上下文隔离**：各 Agent 只接收自己需要的信息切片
- **可独立调优**：压缩模型和知识模型可以使用更便宜/更快的模型
- **失败隔离**：后处理失败不阻断用户可见的叙事输出
...
| Parser | 理解 | 用户输入 + 最近 3 条对话 | 可配置（`parser_enabled`） |
| `npc_{角色名}` | 执行 | 角色档案 + **本轮 `compressed_input`** + scene_summary + events + foreshadowing + structured_state + 知识边界过滤后的 knowledge events（感知流）+ 注入的 Agent 输出 + recall 上下文 | 动态（创建 → 活跃 → 冷却 → 退休/死亡） |
| User Agent | 执行 | 用户角色设定 + **本轮 `compressed_input`** + scene_summary + events（感知流过滤） | 按需激活（`user_auto=true` 时 Runtime 自动注入） |
| Writer | 合成 | 文风预设 + scene_summary + foreshadowing + 本轮 Agent 输出 | 永久 |
| State (变量) | 变量更新 | _state_agent_writable 变量 + NPC/User Agent 输出 | Runtime 自动注入（当变量存在时） |
...
Turn N:
  ...
  压缩 Agent → scene_summary + events + foreshadowing + state_changes
    → 存入各自表
  知识抽取 → agent_knowledge_events（谁知道什么）
    → 存入表

Turn N+1:
  ContextBundle 从 state/events/foreshadowing/summaries/recall 表读取
  （角色 Agent——NPC/User——不从 messages 表读取原始对话用于推理，只收 compressed_input + 感知流；
   合成器 Agent——Writer/Director/Master——例外，仍读 recent_context 维持叙事文风/GM 视野。详见下表。）
```

### 上下文注入策略（按 Agent 类型）

> **角色 Agent vs 合成器 Agent**：心智隔离（防全知）只针对**角色 Agent**——NPC 与 User 不收原始对话历史，只收本轮 `compressed_input` + 感知流。**Writer / Director 是全知合成器**（Writer = 叙事者、Director = GM），全知是其设计职责，且 Writer 需要原文叙事维持文风连续性，因此**它们仍收 `recent_context`**。本表「不收到的」一列对 Writer/Director 不适用（仅约束 NPC/User）。

| Agent 类型 | 收到的上下文 | 不收到的（仅约束 NPC/User） |
|---|---|---|
| NPC | **本轮 `compressed_input`**（Parser 压缩意图，注入 user 消息）+ scene_summary + events + foreshadowing + structured_state + recall 上下文 + 知识边界过滤后的 knowledge events（感知流，注入「已知事实」段） | 原始对话历史、其他 NPC 内心、writer_only 知识 |
| Writer | scene_summary + foreshadowing + **`recent_context`** + 本轮其他 Agent 输出 | 完整 events（其余全可见） |
| Director | scene_summary + events + structured_state + **`recent_context`** | —（GM 全可见） |
| User Proxy | **本轮 `compressed_input`** + scene_summary + events（最近 5 条，感知流过滤） | 原始对话历史、完整 events、foreshadowing |
| Master | 结构化意图 + agent 状态摘要 + scene_summary + events + foreshadowing + **`recent_context`**（尾部优先截断） | 完整状态 JSON |

### Recall 系统

子 Agent 在构建 ContextBundle 时会从 `structured_events` 表检索相关历史事件。支持两种模式：

| 模式 | 说明 |
|---|---|
| `keyword`（默认） | 从用户输入提取关键词，LIKE 搜索 `structured_events`，合并最近 N 条作为 fallback |
| `embedding` | 待实现，当前 fallback 到 keyword 模式 |

每个 Agent 通过 `AgentConfig` 可独立配置 `recall_mode` 和 `max_recall_events`。

### Preset 模块注入

会话可绑定 Preset（提示词预设），Preset 的模块（`preset_modules`）在构建 Agent 的 system prompt 时注入。切换 Preset 影响下一次生成的提示词结构。

### Knowledge 边界系统

每轮 post-commit 阶段，从用户输入 + 最终叙事中抽取"谁知道什么"事件（`KnowledgeEvent`），存入 `agent_knowledge_events` 表。每个事件包含：

| 字段 | 说明 |
|---|---|
| `fact` | 事实描述 |
| `source_type` | `speech` / `action` / `visual_observation` / `inner_monologue` / `narration` / `inference` |
| `actors` / `targets` / `observers` | 行动主体、被作用对象、在场观察者 |
| `knowers` | 知道该事实的角色名列表 |
| `visibility` | `public` / `private` / `writer_only` 等 |
| `confidence` | 置信度 (0.0-1.0) |
| `evidence` | 简短证据原文 |

知识事件在构建子 Agent 上下文时通过 `visible_to_agent` 函数过滤：Writer/Director/Master/State/Parser 可见所有事件；NPC 仅可见 `public` 事件或自己在 `knowers` 列表中的事件；`writer_only` 事件不进入任何 NPC 上下文。详见 Agent 边界与权限。
...
| `variable_tool_model` | 同 sub_agent_model | Variable Tool Agent 使用的模型（可独立配置） |
| `cooldown_turns` | 10 | 多少轮不活跃进入冷却 |
| `user_auto_mode` | `false` | User Agent 自动模式（`boolean`：`true`= 自动生成用户行为，`false`= 不自动） |
