...
6. **Per-Agent 模型覆盖**：优先级为 `agent.config.model` → session `sub_agent_model` → 全局默认模型
...
从用户输入 + 最终叙事中抽取"谁知道什么"事件（`KnowledgeEvent`），写入 `agent_knowledge_events` 表。详见 [Agent 边界与权限](agent-boundaries.md)。
...
| `npc_{角色名}` | 执行 | 角色档案 + scene_summary + events + foreshadowing + structured_state + 注入的 Agent 输出 + recall 上下文 | 动态（创建 → 活跃 → 冷却 → 退休/死亡） |
...
|---|---|---|
| NPC | scene_summary + events + foreshadowing + structured_state + recall 上下文 + 知识边界过滤后的 knowledge events | 原始对话历史、其他 NPC 内心、writer_only 知识 |
...
知识事件在构建子 Agent 上下文时通过 `visible_to_agent` 函数过滤：Writer/Director/Master/State/Parser 可见所有事件；NPC 仅可见 `public` 事件或自己在 `knowers` 列表中的事件；`writer_only` 事件不进入任何 NPC 上下文。详见 [Agent 边界与权限](agent-boundaries.md)。
