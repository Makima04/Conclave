...
1. **Recent Context / 最近上下文** — 最近几轮完整消息，用于保持当下对话的自然衔接。窗口有限，不能承载长期事实。
2. **Scene Summary / 场景摘要** — 当前场景内的滚动摘要，包括地点、参与者、当前冲突、未完成动作。
3. **Chapter Summary / 章节摘要** — 跨场景摘要，用于小说章节、长 RP 阶段、剧情大段落压缩。
4. **Structured State / 结构化状态** — 人物位置、生命状态、物品、关系、目标、已知信息、世界时间、任务状态。它是长期一致性的主账本。
5. **Event Ledger / 事件账本** — 关键事件按时间线入账，记录发生了什么、谁参与、影响了哪些状态、是否公开。
6. **Foreshadowing Registry / 伏笔登记表** — 登记伏笔、谜团、承诺、隐藏秘密、未来回收点，状态包括 open、hinted、triggered、resolved、abandoned。
...
- `PersonalityCore`：稳定人格核心和 OOC 红线。
- `PersonalityState`：当前情绪、关系倾向、短期信念和行为倾向。
- `GrowthArc`：成长方向、阶段、触发事件、允许变化范围。
- `ChangeJustification`：人格变化的证据链。
...
| `ConsistencyIssue` | 记录冲突和 OOC 风险。 | 类型、严重度、相关证据、建议修复、是否已处理。 |
