...
| 检索 | `SQLite LIKE`（当前） | 先用关键词 LIKE 查询解决大部分旧细节召回，避免过早引入复杂向量库。文档推荐 SQLite FTS，但实际实现使用 LIKE 查询。 | 后续接 SQLite FTS5 或 Qdrant / LanceDB 做向量记忆。 |
...
| 长列表 | `TanStack Virtual` | 聊天记录和 artifact 历史必须虚拟化，避免 100+ 轮后 DOM 堆积。**当前未引入**，消息列表无虚拟化，长会话存在性能风险。 | React Virtuoso 也可。 |
...
- `runtime`：Agent Graph 执行、节点调度、上下文装配、LLM 并发限制、Knowledge Events。
- `memory`：摘要、事件、状态、伏笔、检索。
- `importer`：角色卡、世界书导入管线（原 `packages`，已重命名）。世界书包导入待实现。
- `routes`：API 端点（sessions、messages、charactercards、worldbooks、presets、settings、card_import 等）。
- `providers`：OpenAI、OpenRouter、Ollama、vLLM、LM Studio adapter。
- `db`：数据库初始化与迁移。
- `error`：统一错误模型。

> **注意：** `artifacts`（artifact 版本、patch、资源和快照）和 `trace`（每轮调用链记录）在规范中定义，但当前尚未作为独立模块实现。Artifact Renderer 留待 P3。
...
- Rust + axum + tokio + sqlx + SQLite。（已采用）
- React + TypeScript + Vite。（已采用）
- React Flow + TanStack Virtual + Monaco Editor。（均未引入，待需要时按需安装）
- Artifact Renderer：iframe sandbox + state diff。（待实现，P3）
- 长期记忆：结构化状态 + 事件账本 + 伏笔登记表。（已部分实现）

### 已采用的后端依赖

| 依赖 | 用途 |
|---|---|
| `axum` 0.8 | HTTP 框架，含 multipart 支持 |
| `tokio` 1 | 异步运行时 |
| `sqlx` 0.8 | SQLite 数据库访问 |
| `serde` + `serde_json` | JSON 序列化/反序列化 |
| `uuid` 1 | 唯一标识符生成 |
| `chrono` 0.4 | 时间处理 |
| `tower-http` 0.6 | CORS、trace、header 中间件 |
| `tracing` + `tracing-subscriber` | 结构化日志 |
| `reqwest` 0.12 | HTTP 客户端（LLM API 调用） |
| `async-stream` 0.3 | 异步流式处理 |
| `thiserror` 2 | 错误类型派生 |
| `futures` 0.3 | 异步组合子 |
| `async-trait` 0.1 | 异步 trait 支持 |
| `aes-gcm` / `pbkdf2` / `rand` / `base64` / `sha2` | 加密与哈希 |
...
| `vue` 3 | 部分组件使用（角色卡 sandbox 渲染） |

> **未引入的推荐依赖：** Vitest（前端测试）、MSW（Mock Service Worker）、criterion（性能基准）、React Flow（图编辑器）、TanStack Virtual（虚拟列表）、Monaco Editor（代码编辑器）。
