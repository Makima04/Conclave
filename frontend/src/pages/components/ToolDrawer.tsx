...
function ParamsTab(props: ToolDrawerProps) {
  const {
    config, setConfig, configDirty, providers,
    paramsEditing, setParamsEditing,
    saveConfig, updateConfig, applyGlobalDefaultsToSession, saveCurrentSessionAsGlobalDefaults,
  } = props;
...
function WorldbookTab(props: ToolDrawerProps) {
  const navigate = useNavigate();
  const {
    sessionId, characterCard, worldBooks, activeWorldBookId, sessionResourceSaving,
    activeWorldBook,
    updateSessionWorldBook, loadSessionResources,
  } = props;
...
function PresetTab(props: ToolDrawerProps) {
  const navigate = useNavigate();
  const {
    sessionId, config, presets, sessionResourceSaving,
    activePreset, activePresetMissing,
    updateSessionPreset, loadSessionResources,
  } = props;
...
function AgentsTab(props: ToolDrawerProps) {
  const navigate = useNavigate();
  const { sessionId, sessionMode } = props;

  if (sessionMode !== 'multi_agent') return null;
...
function UserTab(props: ToolDrawerProps) {
  const {
    config, userPersona, userPresets, userEditing, setUserEditing,
    updateUserPersona, updateUserSettingMergeStrategy,
    applyUserPersonaPreset, saveConfig,
  } = props;
...
function DebugTab(props: ToolDrawerProps) {
  const {
...
export const ToolDrawer = React.memo(function ToolDrawer(props: ToolDrawerProps) {
  const { open, activeTab, onClose } = props;

  return (
    <aside className={`tool-drawer${open ? ' drawer-visible' : ''}`}>
      <div className="tool-drawer-titlebar">
        <strong>{TAB_LABELS[activeTab] || '设置'}</strong>
        <button type="button" onClick={onClose} title="关闭">×</button>
      </div>
      <div className="tool-drawer-content">
        {activeTab === 'params' && <ParamsTab {...props} />}
        {activeTab === 'worldbook' && <WorldbookTab {...props} />}
        {activeTab === 'preset' && <PresetTab {...props} />}
        {activeTab === 'agents' && <AgentsTab {...props} />}
        {activeTab === 'render' && <RenderTab {...props} />}
        {activeTab === 'user' && <UserTab {...props} />}
        {activeTab === 'debug' && <DebugTab {...props} />}
      </div>
    </aside>
  );
});
