...
// ──────────────────────────────────────────────
console.log('\n--- Test 4: Invalid regex falls back to literal matching ---');
{
  const card = {
    extensions: {
      regex_scripts: [
  ...
  // P1-3: /[invalid/g is now escaped as a literal string, so it compiles
  // successfully and no longer produces a diagnostic. It does not match
  // "ok fine" though, so only the second script runs.
  assertEq(result.diagnostics.length, 0, 'no diagnostics (escape fallback succeeded)');
...
console.log('\n--- Test 8: parseFindRegex helper ---');
{
  ...
  assert(re3 !== null, 'invalid regex falls back to literal string match');
  assertEq(re3?.test('[bad'), true, 'literal fallback matches the escaped pattern');
